...
def _gpt_oss_overrides(server_args: Any, hf_config: Any) -> dict:
    ...
    if server_args.moe_runner_backend == "auto":
        ...
        elif is_hip() and is_mxfp4_quant_format and is_triton_kernels_available():
            # ROCm without aiter (e.g. RDNA/gfx11xx/gfx12xx, where aiter has no
            # MXFP4 compute path): run MXFP4 MoE through the OpenAI triton_kernels
            # path, matching vLLM's ROCm route. Reached only when aiter is off,
            # since the aiter+mxfp4 branch above takes priority.
