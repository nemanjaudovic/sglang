...
struct TopkConstants {
  static constexpr int ELTS_PER_LDG = BYTES_PER_LDG / sizeof(T);
  static_assert(
      EXPERTS / (ELTS_PER_LDG * WARP_SIZE_PARAM) == 0 || EXPERTS % (ELTS_PER_LDG * WARP_SIZE_PARAM) == 0, "");
...
// Warp size is a compile-time literal on CUDA and a runtime
// value on ROCm, so it must be threaded through as a template parameter.
#ifndef USE_ROCM
#define LAUNCH_SIGMOID(TYPE, NUM_EXPERTS, WARPS_PER_TB)                              \
  ...
  topkGatingSigmoidLauncherHelper<TYPE, NUM_EXPERTS, WARPS_PER_TB, WARP_SIZE>(       \
      gating_output,                                                                 \
      nullptr,                                                                       \
      topk_weights,                                                                  \
      topk_indices,                                                                  \
...
#define LAUNCH_SIGMOID(TYPE, NUM_EXPERTS, WARPS_PER_TB)                              \
  if (WARP_SIZE == 64) {                                                             \
    topkGatingSigmoidLauncherHelper<TYPE, NUM_EXPERTS, WARPS_PER_TB, 64>(            \
        gating_output, nullptr, topk_weights, topk_indices, num_tokens, topk, 0,    \
        num_experts, renormalize, correction_bias, stream);                         \
  } else if (WARP_SIZE == 32) {                                                      \
    topkGatingSigmoidLauncherHelper<TYPE, NUM_EXPERTS, WARPS_PER_TB, 32>(            \
        gating_output, nullptr, topk_weights, topk_indices, num_tokens, topk, 0,    \
        num_experts, renormalize, correction_bias, stream);                         \
  } else {                                                                          \
