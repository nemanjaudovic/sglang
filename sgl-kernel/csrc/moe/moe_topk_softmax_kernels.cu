...
void topkGatingSoftmaxLauncherHelper(
  ...
  topkGatingSoftmax<T, VPT, EXPERTS, WARPS_PER_TB, BYTES_PER_LDG, WARP_SIZE_PARAM><<<num_blocks, block_dim, 0, stream>>>(
      input,
      finished,
      output,
      num_rows,
...
// WARP_SIZE is a compile-time literal (32) on CUDA and a runtime value on ROCm
